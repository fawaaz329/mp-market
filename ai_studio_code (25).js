/**
 * MP MARKET BACKEND
 * Cloudflare Worker + D1 + R2 + Durable Objects + Yoco Checkouts + Brevo Transactional Emails + Site-Wide Google Auth
 */
...
// BREVO EMAIL DISPATCHER (WITH REPLY-TO POINTING TO GMAIL)
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      // 🔵 SITE-WIDE GOOGLE AUTH VERIFIER
      if (url.pathname === "/api/auth/google" && request.method === "POST") {
        try {
          ...
          return json({
            user: {
              name: tokenData.name || "Valued Customer",
      ...
      // 🖼️ R2 IMAGE SERVING (Handles %2F, products/ prefix, clean fallback)
      if (
