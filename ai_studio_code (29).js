/**
 * MP MARKET BACKEND
 * Cloudflare Worker + D1 + R2 + Durable Objects + Yoco + Brevo + Google Auth + Directory
 */
  ...
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (url.pathname === "/api/config" && request.method === "GET") {
        ...
        return json({
          delivery_base_fee: parseFloat(settingsMap.delivery_base_fee) || 40.0,
          free_delivery_threshold: parseFloat(settingsMap.free_delivery_threshold) || 500.0,
      ...
      // CHECKOUT SESSION CREATOR (WITH R 200.00 MINIMUM ORDER ENFORCEMENT)
      if (url.pathname === "/api/orders/create-checkout" && request.method === "POST") {
        ...
        const freeThreshold = settingsMap.free_delivery_threshold || 500.0;
        ...
        // SERVER-SIDE MINIMUM ORDER AMOUNT ENFORCEMENT
        const MINIMUM_ORDER_AMOUNT = 200.00;
        if (computedSubtotal < MINIMUM_ORDER_AMOUNT) {
          return json({ error: `The Minimum Order Amount is R ${MINIMUM_ORDER_AMOUNT.toFixed(2)}. Please add more items to your basket.` }, 400);
        }
      ...
      if (url.pathname === "/api/directory/reviews" && request.method === "POST") {
        ...
        if (!listing_id || !customer_name || !rating) return json({ error: "Missing required rating fields" }, 400);
