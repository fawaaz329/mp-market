/**
 * MP MARKET BACKEND
 * Cloudflare Worker + D1 + R2 + Durable Objects + Yoco Checkouts + Free Delivery Threshold Rules
 */
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      // CHECKOUT SESSION GENERATOR (WITH FREE DELIVERY THRESHOLD LOGIC)
      ...
      // CONFIRM PAYMENT & DISPATCH EMAILS
      if (url.pathname === "/api/orders/confirm-payment" && request.method === "POST") {
        ...
        if (order.payment_status !== "Paid") {
          ...
          // Send Email Receipt if MailChannels configured
          if (order.customer_email) {
            ctx.waitUntil(sendMailChannelsEmail(env, {
              to: order.customer_email,
              toName: order.customer_name,
              subject: `Order Confirmed: #${order.order_number} — MP Market`,
              htmlContent: `<p>Hi ${order.customer_name}, your order #${order.order_number} is confirmed. Total: R ${Number(order.total_amount).toFixed(2)}</p>`
            }));
          }
        }
