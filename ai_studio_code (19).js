...
// -------------------------------------------------------------
// BREVO (SENDINBLUE) OFFICIAL REST API DISPATCHER
// -------------------------------------------------------------
async function sendSystemEmail(env, { to, toName, subject, htmlContent }) {
  try {
    const apiKey = (env.BREVO_API_KEY || "").trim();
    if (!apiKey) {
      console.error("BREVO_API_KEY is missing in Cloudflare Secrets.");
      return false;
    }
  ...
  return `
    <div style="font-family: Arial, sans-serif; background-color: #F7F4EF; padding: 24px 16px; color: #171717;">
      <div style="max-width: 580px; margin: 0 auto; background: #FFFFFF; border-radius: 8px; border: 2px solid #C45A2C; padding: 24px;">
        ...
        <h3 style="font-size: 14px; margin: 0 0 10px 0; text-transform: uppercase;">🛒 Items to Pick Up:</h3>
        ...
        <div style="background: #FAF7F2; padding: 12px; border-radius: 6px; font-size: 13px; margin-bottom: 20px;">
          ...
          <div><strong>Delivery Fee:</strong> ${order.delivery_fee === 0 ? 'FREE / Collection' : 'R ' + Number(order.delivery_fee).toFixed(2)}</div>
        </div>
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      // CONFIRM PAYMENT & DISPATCH BREVO EMAILS
      if (url.pathname === "/api/orders/confirm-payment" && request.method === "POST") {
        ...
        if (order.payment_status !== "Paid") {
          ...
          // ✉️ DISPATCH EMAIL 1: Customer Receipt via Brevo
          ...
          // ✉️ DISPATCH EMAIL 2: Owner Alert via Brevo
          const ownerEmail = env.OWNER_NOTIFICATION_EMAIL || env.ADMIN_EMAIL;
          if (ownerEmail) {
            ctx.waitUntil(
              sendSystemEmail(env, {
