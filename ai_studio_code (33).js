...
async function sendSystemEmail(env, { to, toName, subject, htmlContent }) {
  try {
    ...
    if (!apiKey) {
      console.error("[Email Error] Missing BREVO_API_KEY");
      return false;
    }
    ...
    const res = await fetch("https://api.brevo.com/v3/smtp/email", {
      ...
      body: JSON.stringify({
        ...
        replyTo: { email: recipientEmail, name: fromName },
        to: [{ email: recipientEmail, name: toName || recipientEmail }],
        subject: subject,
    ...
    if (!res.ok) {
      ...
      console.error("[Brevo SMTP Error]", res.status, errText);
    }
    return res.ok;
  } catch (err) {
    console.error("[System Email Exception]", err);
    return false;
  }
}
...
function renderDirectorySubmissionEmail({ business_name, category, suburb, owner_email, whatsapp_number, is_update }) {
  return `
    <div style="font-family: Arial, sans-serif; padding: 24px; color: #171717; max-width: 580px; margin: 0 auto; background: #FFFFFF; border: 1px solid #E8E3DA; border-radius: 8px;">
      <h2 style="color: #087A58; margin-top: 0;">${is_update ? 'Directory Listing Updated' : 'New Community Directory Listing Submitted'}</h2>
      <p>A local seller has submitted their business profile for approval on MP Market:</p>
      ...
      <div style="background: #F7F4EF; padding: 16px; border-radius: 6px; margin: 16px 0; font-size: 13px; line-height: 1.6;">
        ...
        <div><strong>WhatsApp:</strong> ${whatsapp_number}</div>
        <div><strong>Status:</strong> Pending Admin Approval</div>
      </div>

      <p style="font-size: 13px; color: #555;">
        Log into your <strong>Admin Dashboard &rarr; Directory Approvals</strong> to inspect their business bio and gallery photos before approving.
      </p>
    </div>
  `;
}
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      // CONFIG WITH DYNAMIC MINIMUM ORDER, HIDDEN CATEGORIES & MITCHELL'S PLAIN ONLY MODE
      ...
      if (url.pathname === "/api/contact" && request.method === "POST") {
        ...
        const ownerEmail = env.OWNER_NOTIFICATION_EMAIL || env.ADMIN_EMAIL || env.SENDER_EMAIL || "orders@mp-marketplace.co.za";
        if (ownerEmail) {
          ctx.waitUntil(
            sendSystemEmail(env, {
              to: ownerEmail,
      ...
      // CHECKOUT SESSION CREATOR (YOCO) WITH DYNAMIC DATABASE MINIMUM CHECK
      if (url.pathname === "/api/orders/create-checkout" && request.method === "POST") {
        ...
        const minOrderThreshold = settingsMap.min_order_amount ? parseFloat(settingsMap.min_order_amount) : 200.0;
        ...
        if (computedSubtotal < minOrderThreshold) {
          return json({ error: `Minimum order value is R${minOrderThreshold.toFixed(2)}. Your current basket subtotal is R ${computedSubtotal.toFixed(2)}.` }, 400);
        }
      ...
      if (url.pathname.startsWith("/api/admin")) {
        ...
        if (url.pathname === "/api/admin/areas" && request.method === "POST") {
          ...
          ).bind(areaId, name.trim(), zone_name || 'Mitchells Plain', Number(fee || 0), is_collection ? 1 : 0).run();
        ...
        if (url.pathname.startsWith("/api/admin/areas/") && request.method === "PUT") {
          ...
          ).bind(name.trim(), zone_name || 'Mitchells Plain', Number(fee || 0), is_collection ? 1 : 0, id).run();
      ...
      if (url.pathname.startsWith("/api/directory/contact/") && request.method === "GET") {
        ...
        let message = `Hello ${listing.business_name}, I found your business profile on the MP Market Community Directory. Are your services/products currently available?`;
        if (itemTitle) {
          message = `Hello ${listing.business_name}, I am interested in ${itemTitle}${itemPrice ? ` (R${itemPrice})` : ''} as seen on your MP Market profile.`;
        }
