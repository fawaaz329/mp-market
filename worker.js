/**
 * MP MARKET BACKEND
 * Cloudflare Worker + D1 + R2 + Durable Objects + Yoco + MailChannels Transactional Emails
 */
  ...
  try {
    ...
    const payload = {
      ...
      content: [
        {
          type: "text/plain",
          value: textContent || "MP Market notification"
        },
        {
          type: "text/html",
          value: htmlContent
    ...
    <div style="font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; background-color: #F7F4EF; padding: 32px 16px; color: #171717;">
      <div style="max-width: 580px; margin: 0 auto; background: #FFFFFF; border-radius: 8px; border: 1px solid #E8E3DA; padding: 32px; box-shadow: 0 4px 14px rgba(0,0,0,0.04);">
        ...
        <p style="font-size: 14px; color: #404040; line-height: 1.5; margin-bottom: 24px;">
          Thank you for ordering with <strong>MP Market</strong>! We have received your order, and our local concierge team is now preparing to source your items.
        </p>
    ...
    <div style="font-family: Arial, sans-serif; background-color: #F7F4EF; padding: 32px 16px; color: #171717;">
      <div style="max-width: 540px; margin: 0 auto; background: #FFFFFF; border-radius: 8px; border: 1px solid #E8E3DA; padding: 32px;">
        ...
        <p style="font-size: 13px; color: #404040; line-height: 1.5; margin-bottom: 24px;">
          Please ensure someone is available at the address to receive the parcel. If you need to send specific gate instructions or gate codes, reply directly to this email or send us a WhatsApp message.
        </p>
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      // ORDER CHECKOUT (DISPATCHES EMAIL 1 & EMAIL 2)
      if (url.pathname === "/api/orders" && request.method === "POST") {
        const body = await request.json();
        const { customer_name, customer_email, customer_whatsapp, delivery_address, items, yoco_token } = body;
        ...
        const emailToSave = customer_email ? customer_email.trim() : "";
        ...
        const statements = [
          env.DB.prepare(
            `INSERT INTO orders (id, order_number, customer_name, customer_email, customer_whatsapp, delivery_address, subtotal_amount, delivery_fee, total_amount, status, payment_status, payment_method) 
             VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`
          ).bind(orderId, orderNumber, customer_name, emailToSave, customer_whatsapp, delivery_address, computedSubtotal, deliveryFee, computedTotal, initialStatus, paymentStatus, paymentMethod)
        ];
        ...
        await doObj.fetch(new Request("https://realtime/broadcast", {
          method: "POST",
          body: JSON.stringify({ event: "NEW_ORDER", order: orderObject })
        }));

        // DISPATCH EMAIL 1: Customer Order Confirmation
        if (emailToSave) {
          ctx.waitUntil(
            sendMailChannelsEmail(env, {
              to: emailToSave,
              toName: customer_name,
              subject: `Order Confirmed: #${orderNumber} — MP Market`,
              htmlContent: renderCustomerOrderEmail(orderObject, verifiedItems)
            })
          );
        }

        // DISPATCH EMAIL 2: Business Owner Alert
        ...
        if (ownerEmail) {
          ctx.waitUntil(
            sendMailChannelsEmail(env, {
              ...
              subject: `🚨 New Sourcing Order: #${orderNumber} — ${customer_name} (R ${Number(computedTotal).toFixed(2)})`,
              htmlContent: renderOwnerAlertEmail(orderObject, verifiedItems)
            })
          );
        }
      ...
      // ADMIN ROUTES (STATUS UPDATES TRIGGER EMAIL 3 & EMAIL 4)
      if (url.pathname.startsWith("/api/admin")) {
        ...
        // UPDATE ORDER STATUS (TRIGGERS EMAIL 3 & EMAIL 4)
