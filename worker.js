...
// BREVO EMAIL DISPATCHER (REPLY-TO POINTING TO GMAIL)
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      // 🔵 GOOGLE OAUTH ORDER SYNC (STRIPS ALL SENSITIVE DATA FROM PUBLIC PAYLOAD)
      ...
      // 🖼️ R2 IMAGE SERVING (Handles %2F, clean prefixes)
      if (
      ...
      // PRODUCTS WITH cat-all FIX (CONTAINS ONLY PRODUCT DATA - ZERO CUSTOMER INFO)
      ...
      // ADMIN CORE ROUTES (PROTECTED BY ADMIN PASSWORD)
      ...
      // 1. PUBLIC DIRECTORY SEARCH (ZERO PHONE NUMBERS, ZERO EMAILS IN JSON)
      if (url.pathname === "/api/directory/listings" && request.method === "GET") {
        ...
        // STRICT COLUMN SELECTION: Excludes owner_email AND whatsapp_number!
        let query = `
      ...
      // 2. SECURE WHATSAPP REDIRECT RELAY (PHONE NUMBER IS NEVER EXPOSED TO JAVASCRIPT)
      if (url.pathname.startsWith("/api/directory/contact/") && request.method === "GET") {
        ...
        // 302 Redirection straight into WhatsApp app
      ...
      // 4. SELLER'S OWN LISTING (ONLY ACCESSIBLE TO VERIFIED OWNER)
