/**
 * MP MARKET BACKEND
 * Cloudflare Worker + D1 + R2 + Durable Objects + Yoco Checkouts + Automated Emails
 */
...
// -------------------------------------------------------------
// RELIABLE EMAIL DISPATCHER (MailChannels + Resend Fallback)
// -------------------------------------------------------------
async function sendSystemEmail(env, { to, toName, subject, htmlContent, textContent }) {
  try {
    ...
    // 2. Default to MailChannels relay
    const payload = {
      personalizations: [{ to: [{ email: to, name: toName || to }] }],
      from: { email: fromEmail, name: fromName },
      subject: subject,
      content: [
        { type: "text/plain", value: textContent || "MP Market Order Notification" },
    ...
    <div style="font-family: Arial, sans-serif; background-color: #F7F4EF; padding: 32px 16px; color: #171717;">
      <div style="max-width: 580px; margin: 0 auto; background: #FFFFFF; border-radius: 8px; border: 1px solid #E8E3DA; padding: 32px;">
        ...
        <div style="background: #FAF7F2; border: 1px solid #E8E3DA; border-radius: 6px; padding: 16px; margin-bottom: 24px; font-size: 13px;">
          <div style="margin-bottom: 6px;"><strong>Order Number:</strong> <span style="color: #C45A2C; font-weight: 800;">${order.order_number}</span></div>
    ...
    <div style="font-family: Arial, sans-serif; background-color: #F7F4EF; padding: 24px 16px; color: #171717;">
      <div style="max-width: 580px; margin: 0 auto; background: #FFFFFF; border-radius: 8px; border: 2px solid #C45A2C; padding: 24px;">
        ...
        <div style="background: #FAF7F2; padding: 12px; border-radius: 6px; font-size: 13px; margin-bottom: 20px;">
          ...
          <div><strong>Delivery Fee Earned:</strong> ${order.delivery_fee === 0 ? 'FREE / Collection' : 'R ' + Number(order.delivery_fee).toFixed(2)}</div>
          <div><strong>Payment Status:</strong> Paid via Yoco Online</div>
        </div>

        <p style="font-size: 12px; color: #737373; margin: 0;">Log into your Control Portal at /admin.html to print your single-page waybill.</p>
      </div>
    </div>
  `;
}
  ...
  return `
    <div style="font-family: Arial, sans-serif; background-color: #F7F4EF; padding: 32px 16px; color: #171717;">
      <div style="max-width: 540px; margin: 0 auto; background: #FFFFFF; border-radius: 8px; border: 1px solid #E8E3DA; padding: 32px;">
        ...
        <p style="font-size: 13px; color: #404040; line-height: 1.5; margin-bottom: 24px;">Please ensure someone is available at the address to receive the parcel.</p>
    ...
    <div style="font-family: Arial, sans-serif; background-color: #F7F4EF; padding: 32px 16px; color: #171717;">
      <div style="max-width: 540px; margin: 0 auto; background: #FFFFFF; border-radius: 8px; border: 1px solid #E8E3DA; padding: 32px;">
        ...
        <div style="background: #FAF7F2; border-left: 4px solid #C45A2C; padding: 14px 16px; border-radius: 4px; font-size: 13px; color: #404040; line-height: 1.5; margin-bottom: 24px;">
          ...
          By ordering through MP Market, you are directly supporting local suppliers and community couriers in Mitchells Plain.
        </div>
        <div style="text-align: center; padding: 16px; background: #FAFAFA; border: 1px solid #E8E3DA; border-radius: 6px; margin-bottom: 24px;">
          ...
          <p style="font-size: 12px; color: #737373; margin: 0;">We would love to hear your thoughts! Visit our store to leave a quick star review on your items.</p>
        </div>
        <p style="font-size: 13px; font-weight: 700; color: #171717; margin: 0;">The MP Market Team</p>
      </div>
    </div>
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      // CONFIRM PAYMENT & AUTOMATICALLY SEND EMAIL 1 & EMAIL 2
      if (url.pathname === "/api/orders/confirm-payment" && request.method === "POST") {
        ...
        if (order.payment_status !== "Paid") {
          ...
          // ✉️ DISPATCH EMAIL 1: Customer Order Receipt
          ...
          // ✉️ DISPATCH EMAIL 2: Business Owner Sourcing Alert
          const ownerEmail = env.OWNER_NOTIFICATION_EMAIL || env.ADMIN_EMAIL;
          if (ownerEmail) {
            ctx.waitUntil(
              sendSystemEmail(env, {
      ...
      if (url.pathname.startsWith("/api/admin")) {
        ...
        // UPDATE STATUS & DISPATCH EMAIL 3 & EMAIL 4
