...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      // ⚡ WEBSOCKET REALTIME (Supports both /api/ws and /api/ws/admin)
      ...
      // 🖼️ FIX 2: ROBUST R2 IMAGE RETRIEVER (Cleans %2F and handles /api/images, /images, & /products)
      if (
        ...
        // Remove any leading slashes or url-encoded %2F
        let cleanKey = rawKey.replace(/^[\/\%2F]+/, "").trim();

        // 1. Try directly with cleanKey
        let object = await env.IMAGES_BUCKET.get(cleanKey);

        // 2. Fallback: Check inside "products/" folder if not prefixed
        if (!object && !cleanKey.startsWith("products/")) {
          object = await env.IMAGES_BUCKET.get(`products/${cleanKey}`);
        }

        // 3. Fallback: Strip "products/" if saved with a flat key
        if (!object && cleanKey.startsWith("products/")) {
      ...
      // 📦 FIX 1: MATCHES BOTH /api/products AND /products + HANDLES cat-all GRACEFULLY
      ...
      if (url.pathname.startsWith("/api/admin")) {
        ...
        // 📷 UPLOAD (Saves standard clean path)
