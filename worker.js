/**
 * MP MARKET BACKEND
 * Cloudflare Worker + D1 + R2 + Durable Objects + Product Edits + Area Management + Brevo Emails
 */
  ...
  const itemsHtml = (items || []).map(i => `
    <tr>
      <td style="padding: 10px 0; border-bottom: 1px solid #E8E3DA;">
        ...
        <span style="font-size: 11px; color: #737373;">Source: Local Partner</span>
      </td>
...
function renderOwnerAlertEmail(order, items) {
  const itemsList = (items || []).map(i => `
    <li><strong>[${i.internal_supplier || 'Local Partner'}]:</strong> ${i.quantity}x ${i.product_name} ${i.selected_size ? `(Size: ${i.selected_size})` : ''} - R ${Number(i.unit_price).toFixed(2)}</li>
  `).join('');
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      // 1. WEBSOCKET REALTIME (Connected by both Customers and Admin)
      ...
      // 4. CHECKOUT SESSION CREATOR (WITH SIZES & INTERNAL SUPPLIER LOGIC)
      ...
      if (url.pathname.startsWith("/api/admin")) {
        ...
        if (url.pathname.startsWith("/api/admin/products/toggle/") && request.method === "POST") {
          ...
          // Broadcast real-time catalogue update to customer screens
          const doId = env.REALTIME.idFromName("mp-market-global-room");
          const doObj = env.REALTIME.get(doId);
          await doObj.fetch(new Request("https://realtime/broadcast", {
            method: "POST",
        ...
        // CREATE PRODUCT (WITH REAL-TIME BROADCAST)
        if (url.pathname === "/api/admin/products" && request.method === "POST") {
          ...
          await env.DB.prepare(
            `INSERT INTO products (id, name, description, price, public_source, internal_supplier, available_sizes, is_available, is_new_arrival, sourcing_time, category_id, image_url, is_local_find)
             VALUES (?, ?, ?, ?, ?, ?, ?, 1, ?, ?, ?, ?, 1)`
          ).bind(id, name, description || "", parseFloat(price), public_source || "Local Partner", internal_supplier || "Local Partner", available_sizes || "", is_new_arrival ? 1 : 0, sourcing_time || "Dispatched in 24h", category_id, image_url || "").run();

          // Broadcast silent real-time refresh to all shopping customers
          const doId = env.REALTIME.idFromName("mp-market-global-room");
          const doObj = env.REALTIME.get(doId);
          await doObj.fetch(new Request("https://realtime/broadcast", {
            method: "POST",
        ...
        // EDIT EXISTING PRODUCT (WITH REAL-TIME BROADCAST)
        if (url.pathname.startsWith("/api/admin/products/") && request.method === "PUT") {
          ...
          await env.DB.prepare(
            `UPDATE products 
             ...
             WHERE id = ?`
          ).bind(name, description || "", parseFloat(price), public_source || "Local Partner", internal_supplier || "Local Partner", available_sizes || "", category_id, image_url || "", sourcing_time || "Dispatched in 24h", is_new_arrival ? 1 : 0, id).run();

          // Broadcast silent real-time refresh
          const doId = env.REALTIME.idFromName("mp-market-global-room");
          const doObj = env.REALTIME.get(doId);
          await doObj.fetch(new Request("https://realtime/broadcast", {
            method: "POST",
