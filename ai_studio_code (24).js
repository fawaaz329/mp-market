...
// BREVO EMAIL DISPATCHER (WITH REPLY-TO)
  ...
  try {
    ...
    const res = await fetch("https://api.brevo.com/v3/smtp/email", {
      ...
      body: JSON.stringify({
        ...
        replyTo: { email: "docfloweditor@gmail.com", name: fromName }, // Replies land in your Gmail
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      // CUSTOMER ORDERS ROUTE (MASKS HOME STREET ADDRESS UNDER POPIA)
      if (url.pathname.startsWith("/api/orders/customer/") && request.method === "GET") {
        const query = decodeURIComponent(url.pathname.replace("/api/orders/customer/", "")).trim().toLowerCase();
        
        const { results: orders } = await env.DB.prepare(
          `SELECT * FROM orders 
           WHERE (LOWER(customer_whatsapp) = ? OR LOWER(customer_email) = ?) 
        ...
        const sanitized = (orders || []).map(o => {
          let maskedAddress = "Cape Town";
          if (o.delivery_address) {
            const parts = o.delivery_address.split(',');
            maskedAddress = parts.length > 1 ? `***, ${parts.slice(1).join(',').trim()}` : `*** ${o.delivery_address.slice(-12)}`;
          }
      ...
      if (url.pathname.startsWith("/api/admin")) {
        ...
        // 🗑️ 1-CLICK POPIA ERASURE: PERMANENTLY DELETE SPECIFIC ORDER AND CUSTOMER DETAILS
