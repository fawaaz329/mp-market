/**
 * MP MARKET BACKEND
 * Cloudflare Worker + D1 + R2 + Durable Objects + Yoco Checkouts + Brevo Emails + Google Auth + Directory
 */
...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      ...
      if (url.pathname === "/api/auth/google" && request.method === "POST") {
        try {
          ...
          const { results: orders } = await env.DB.prepare(
            `SELECT * FROM orders WHERE LOWER(customer_email) = ? AND payment_status = 'Paid' ORDER BY created_at DESC`
          ).bind(verifiedEmail).all();
          const { results: allItems } = await env.DB.prepare("SELECT * FROM order_items").all();

          const verifiedOrders = (orders || []).map(o => ({
            ...o,
      ...
      // R2 IMAGE SERVING (Handles %2F and clean prefixes)
      if (
      ...
      if (url.pathname.startsWith("/api/admin")) {
        ...
        // 1-CLICK POPIA ERASURE: PERMANENTLY DELETE SPECIFIC ORDER AND CUSTOMER DETAILS
        ...
        // CREATE PRODUCT (WITH available_colors)
        ...
        // EDIT PRODUCT (WITH available_colors)
        ...
        if (url.pathname.startsWith("/api/admin/directory/") && request.method === "DELETE") {
          ...
          await env.DB.prepare("DELETE FROM directory_listings WHERE id = ?").bind(id).run();
          return json({ success: true });
        }
      }

      // -------------------------------------------------------------
      // 📍 PUBLIC DIRECTORY ENDPOINTS (ISOLATED MODULE)
      // -------------------------------------------------------------
      if (url.pathname === "/api/directory/listings" && request.method === "GET") {
        ...
        let query = "SELECT * FROM directory_listings WHERE status = 'active'";
        const params = [];

        if (shop) {
          query += " AND (slug = ? OR id = ?)";
          params.push(shop, shop);
        } else {
          if (suburb && suburb !== "ALL") {
            query += " AND LOWER(suburb) = ?";
          ...
          if (search && search.trim()) {
            const term = `%${search.trim().toLowerCase()}%`;
            query += " AND (LOWER(business_name) LIKE ? OR LOWER(suburb) LIKE ? OR LOWER(category) LIKE ? OR LOWER(about_text) LIKE ? OR LOWER(showcase_items) LIKE ?)";
            ...
            sendSystemEmail(env, {
              ...
              htmlContent: `
                <div style="font-family: sans-serif; padding: 20px;">
                  ...
                  <p><strong>WhatsApp:</strong> ${cleanPhone}</p>
                  <p>Open your Admin Dashboard to review and tap <strong>Approve</strong> to publish.</p>
                </div>
              `
            })
          );
